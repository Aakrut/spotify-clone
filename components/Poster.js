...
const Poster = ({ track, chooseTrack }) => {
  ...
  return (
    <div
      className="w-[260px] h-[360px] rounded-[40px] overflow-hidden relative text-white/80 cursor-pointer hover:scale-105 hover:text-white/100 transition duration-200 ease-out group mx-auto"
      onClick={handlePlay}
    >
      <img
        src={track.albumUrl}
      ...
      <div className="absolute bottom-10 inset-x-0 ml-4 flex items-center space-x-3.5">
        ...
        <div className="text-[15px]">
          <h4 className="font-extrabold truncate w-44 font-body">{track.title}</h4>
          <h6 className="font-body">{track.artist}</h6>
        </div>
      </div>
    </div>
  );
